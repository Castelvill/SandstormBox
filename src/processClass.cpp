...
void detectRecursionWithRecursion(vector<EventModule> & EventContainer,
    vector<string> & calledEvents, EventModule & Event
){
...
void detectRecursionInEvents(vector<EventModule> & EventContainer,
    const InstrDescription & CurrentInstr
){
    //If a child is called by recursion, don't check its children,
    //because you are either doing it now or you've already done it. 
    ...
    for(EventModule & Event : EventContainer){
        ...
        while(!nodeStack.empty()){
            ...
            //Why? Because you are either doing it right now (an event directly called itself)
...
void ProcessClass::allocateBuiltInVariables(ObjectMemoryStruct &CurrentMap, AncestorObject &Object,
    LayerClass &Layer
){
    if(CurrentMap.topAddress > 0){
        //User-defined global variables will be added to the global scope during the compilation.
        return; 
    }
    ...
    allocateRealMemory("null", null_dt, isReadOnly, isLocal, isReference, builtInVarAddr::NULL_bv,
        0, CurrentMap
    );
    if(allocateRealMemory("self", object_inst, writable, isLocal, isReference,
        builtInVarAddr::me_bv, 0, CurrentMap
    )){
        CurrentMap.MemoryMap.back().Objects.push_back(&Object);
    }
    if(allocateRealMemory("self_layer", layer_inst, writable, isLocal, isReference,
        builtInVarAddr::my_layer_bv, 0, CurrentMap
    )){
        CurrentMap.MemoryMap.back().Layers.push_back(&Layer);
    }
    CurrentMap.topAddress = 3;
}
inline bool wasMemberVariableDefined(const vector<VariableLocationStruct> & GlobalScope,
    const string & variableName, DataType variableType
){
